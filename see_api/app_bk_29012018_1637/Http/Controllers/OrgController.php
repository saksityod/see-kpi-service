...
class OrgController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		if ($all_emp[0]->count_no > 0) {
			...
			$chief_emp_code = Auth::id();
			empty($request->level_id) ? $level = "" : $level = " and a.level_id = " . $request->level_id . " ";
			empty($request->org_code) ? $org = "" : $org = " and a.org_code = " . $request->org_code . " ";
			$items = DB::select("
				select distinct a.org_id, a.org_code, a.org_name, a.org_abbr, a.is_active, b.org_name parent_org_name, a.parent_org_code, a.level_id, c.appraisal_level_name, a.longitude, a.latitude, a.province_code, d.province_name
				from employee z left outer join
				org a on z.org_id = a.org_id
				left outer join
				org b on b.org_code = a.parent_org_code
				left outer join appraisal_level c
				on a.level_id = c.level_id
				left outer join province d on a.province_code = d.province_code
				where 1=1 " . $level . $org . "
				and z.chief_emp_code = {$chief_emp_code}
				order by a.org_code asc
			");		
		}
