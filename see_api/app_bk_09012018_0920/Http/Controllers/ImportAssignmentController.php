...
class ImportAssignmentController extends Controller
{
  ...
  public function level_list(Request $request){
    if ($request->appraisal_type_id == "1") {
      $levels = DB::select("
        SELECT org.level_id, al.appraisal_level_name level_name
        FROM org, appraisal_level al
        WHERE org.level_id = al.level_id
        AND org.is_active = 1
      ...
      $levels = DB::select("
        SELECT emp.level_id, al.appraisal_level_name level_name
        FROM employee emp, appraisal_level al
        WHERE emp.level_id = al.level_id
        AND emp.is_active = 1
        GROUP BY emp.level_id
        ORDER BY emp.level_id
      ");
    } else {
      $levels = [];
    }
