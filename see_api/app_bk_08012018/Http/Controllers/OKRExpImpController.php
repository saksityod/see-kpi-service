...
class OKRExpImpController extends Controller
{
    public function exportMaster(Request $request){
        $filename = "master_import_okr";
    ...
    public function validateTest(Request $request){
        ...
        Log::info($result_obj->result_status);
        Log::info($result_obj->code);
        Log::info($result_obj->msg);
        return response()->json($result_obj, 200);
        //return parent::validateTemplate(1,$f,$bank_values,$number_values);
        /*
        set_time_limit(0);
        $bank_values = [];
        $number_values = ['E','F','G','H','I','J'];
        //$number_values = ['A','D'];
        $appraialValidator = new AppraialValidator();
        $result_obj = $appraialValidator->validateTemplate(1,$request,$bank_values,$number_values);
