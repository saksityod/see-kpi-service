...
class QuestionaireDataController extends Controller
{
    ...
    function next_assess_form($level_id, $data_header_id) {
        $emp_snap = DB::select("
    ...
    function send_email($config, $to_stage_id, $data_header_id, $assessor_id, $status) {
        ...
        if($stage_email->send_email_flag==1) {
            try {
                ...
                $emp_snap = $this->next_assess_form($assessor_id->level_id, $data_header_id);
    ...
    function get_emp_snapshot() {
        try {
            $is_emp = EmployeeSnapshot::select("emp_snapshot_id","level_id","job_function_id")->where("emp_code", Auth::id())->where("is_active", 1)->orderBy('start_date', 'desc')->firstOrFail();
    ...
    function calculate_score($data_header_id) {
        $score_type = DB::select("
            SELECT q.pass_score_type
            FROM questionaire_data_header qd
        ...
        }else if(!empty($score_type) && $score_type[0]->pass_score_type == "score"){
            $cal_score = DB::select("
                SELECT 
                    sum(score) score,
                    sum(full_score) full_score
    ...
    public function assign_template($data_header_id) {
        try {
            $data_header = QuestionaireDataHeader::findOrFail($data_header_id);
        } catch (ModelNotFoundException $e) {
            return response()->json(['status' => 404, 'data' => 'QuestionaireDataHeader not found.']);
        }
        ...
        foreach ($sub_items as $key => $v_qdd) {
            ...
            if($v_qdd->is_cust_search==0) {
                foreach ($sub_items[$key]->sub_section as $key2 => $anv) {
                    $sub_items[$key]->sub_section[$key2]->answer = $this->query_answer($anv->answer_type_id, $anv->question_id, $data_header_id, null);
                }
                ...
                foreach ($sub_items[$key]->sub_section as $key2 => $anv) {
                    ...
                    foreach ($sub_items[$key]->sub_section[$key2]->question as $key3 => $ssq) {
                        $sub_items[$key]->sub_section[$key2]->question[$key3]->answer = $this->query_answer($ssq->answer_type_id, $ssq->question_id, $data_header_id, null);
                    }
                }
            } else {
                foreach ($sub_items[$key]->sub_section as $key2 => $anv) {
    ...
    public function store(Request $request) {
        ...
        if($status==200) {
            ...
            $data_header = $this->next_assess_form($assessor->level_id, $h->data_header_id);
            $data_next_form['data_header_id'] = $h->data_header_id;
            $data_next_form['edit_flag'] = $data_header[0]->edit_flag;
    ...
    public function auto_emp_report(Request $request) {
        ...
        if ($all_emp[0]->count_no > 0) {
            ...
            $emp_code_in = "AND es.emp_code IN ({$in_emp})";
            $assessor_id = $this->get_emp_snapshot();
            $emp_snapshot_id_with_date = $this->get_emp_snapshot_id_with_date($request->start_date, $request->end_date);
            $emp_snapshot_id_with_datee = empty($emp_snapshot_id_with_date[0]->emp_snapshot_id) ? "" : $emp_snapshot_id_with_date[0]->emp_snapshot_id;
            $assessor = "AND (qdh.emp_snapshot_id = '{$emp_snapshot_id_with_datee}' OR qdh.assessor_id = '{$assessor_id->emp_snapshot_id}')";
            ...
            FROM
                ...
                AND (qn.questionaire_type_id = '{$request->questionaire_type_id}' OR '' = '{$request->questionaire_type_id}')
    ...
    // Report
    public function auto_emp_report2(Request $request) {
    ...
    public function list_assessor_report(Request $request) {
        ...
        $questionaire_type_id = empty($request->questionaire_type_id) ? "" : "AND qn.questionaire_type_id = '{$request->questionaire_type_id}'";
        $request->start_date = $this->format_date($request->start_date);
        $request->end_date = $this->format_date($request->end_date);
    ...
    public function list_assessor_transaction(Request $request) {
        $emp_snapshot = empty($request->emp_snapshot_id) ? "" : "AND qdh.emp_snapshot_id = '{$request->emp_snapshot_id}'";
        $request->start_date = $this->format_date($request->start_date);
        $request->end_date = $this->format_date($request->end_date);
