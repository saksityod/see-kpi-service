...
class AppraisalLevel360Controller extends Controller
{
	...
	public function appraisal_criteria(Request $request, $level_id)
	{
		try {
	...
	public function update_criteria(Request $request, $level_id)
	{
		...
		foreach ($request->criteria as $c){
			if ($c['checkbox'] == 1) {
				...
				if($struc->form_id == 2){
					$compCriteria = CompetencyCriteria::where('appraisal_form_id', $request->appraisal_form_id)
		...
		// --> Check the total weight score eq 100% //
		...
		foreach ($request->criteria as $c) {
			if ($c['checkbox'] == 1) {
				$criteria = AppraisalCriteria::where('appraisal_form_id', $request->appraisal_form_id)
					->where('appraisal_level_id',$level_id)
					->where('structure_id',$c['structure_id']);
				if ($criteria->count() > 0) {
					AppraisalCriteria::where('appraisal_form_id', $request->appraisal_form_id)
						->where('appraisal_level_id',$level_id)
						->where('structure_id',$c['structure_id'])
						->update([
